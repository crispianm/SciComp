...
@testset verbose = true "System Unit Tests" begin
    ...
    @testset verbose = true "Finite Difference" begin
        ...
        @testset verbose = true "Input Tests" begin
            ...
            # test error is thrown if method input is incorrect
            @test_throws ErrorException finite_difference(u_I,κ,L,T,mx,mt,
                method = 1)
            @test_throws ErrorException finite_difference(u_I,κ,L,T,mx,mt,
                method = "pseudo_arclengths")
...
if save_figures
    ...
    fd_method_comp = plot([
        scatter(x=0:L/250:L, y=u_exact(0:L/250:L,T), mode="lines", name="Exact Solution", showlegend=true), 
        scatter(x=fe_x, y=fe_u_j, mode="markers", name="Forward Euler", showlegend=true), 
        scatter(x=be_x, y=be_u_j, mode="markers", name="Backward Euler", showlegend=true), 
        scatter(x=cn_x, y=cn_u_j, mode="markers", name="Crank Nicholson", showlegend=true)
        ], layout)
